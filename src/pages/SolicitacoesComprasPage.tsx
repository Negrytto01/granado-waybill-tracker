...
// useRealtime doesn't support this table type, use manual channel
import { formatNF } from "@/lib/helpers";
import { Plus, X, Check, XCircle, Clock, CalendarDays, Send } from "lucide-react";
...
const SolicitacoesComprasPage = () => {
  ...
  const isCompras = profile?.cargo === "Compra" || profile?.cargo === "Compras" || profile?.cargo === "Master";
  const isAgenda = profile?.cargo === "Agendamento/Conferente" || profile?.cargo === "Master";
  ...
  useEffect(() => {
    const channel = supabase
      ...
      .on("postgres_changes", { event: "*", schema: "public", table: "solicitacoes_compras" }, () => {
        fetchData();
      })
      .subscribe();
  ...
  const handleCreate = async () => {
    ...
    const { error } = await supabase.from("solicitacoes_compras").insert([{
      ...
      solicitado_por_user_id: profile?.user_id || (await supabase.auth.getUser()).data.user?.id,
  ...
  const responder = async (aprovado: boolean) => {
    ...
    if (error) { toast.error(error.message); return; }
    toast.success(aprovado ? "Solicitação aprovada! Aguardando confirmação do Compras." : "Solicitação rejeitada.");
    setRespondModal(null);
    setRespostaObs("");
    setRespostaData("");
  };

  const aprovarCompras = async (sol: any) => {
    // Create the recebimento from the solicitação
    ...
    toast.success("Solicitação aprovada e agenda criada automaticamente!");
  };

  const rejeitarCompras = async (sol: any) => {
    await supabase.from("solicitacoes_compras").update({
      status: "REJEITADO_COMPRAS",
      data_aprovacao_compras: new Date().toISOString(),
    } as any).eq("id", sol.id);
    toast.info("Solicitação rejeitada pelo Compras.");
  };
  ...
  const getStatusBadge = (status: string) => {
    ...
    const labels: Record<string, string> = {
      ...
      "APROVADO_AGENDA": "Aguardando Compras",
      "REJEITADO": "Rejeitado pela Agenda",
      "REJEITADO_COMPRAS": "Rejeitado pelo Compras",
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <h1 className="font-heading text-3xl neon-text">Solicitações de Compras</h1>
        {isCompras && (
            ...
            return (
              <div key={sol.id} className="p-4 rounded-lg border border-border bg-card/60 backdrop-blur-sm space-y-3">
                <div className="flex items-center justify-between flex-wrap gap-2">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      ...
                      {sol.horario_sugerido && ` às ${sol.horario_sugerido}`}
                    </p>
                    {nfs.length > 0 && (
                    ...
                    {sol.resposta_observacoes && (
                      <p className="text-xs text-blue-400 mt-1">💬 Resposta: {sol.resposta_observacoes}</p>
                    )}
                ...
                <div className="flex gap-2 flex-wrap">
                  {/* Agenda user can approve/reject PENDENTE */}
                  {sol.status === "PENDENTE" && isAgenda && (
                    <>
                  ...
                  {/* Compras user can confirm/reject APROVADO_AGENDA */}
                  {sol.status === "APROVADO_AGENDA" && isCompras && (
                    <>
                      <Button size="sm" onClick={() => aprovarCompras(sol)} className="bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 hover:bg-emerald-500/30">
                        <Check className="mr-2 h-4 w-4" /> Confirmar e Agendar
                      </Button>
                      <Button size="sm" onClick={() => rejeitarCompras(sol)} className="bg-red-500/20 text-red-400 border border-red-500/30 hover:bg-red-500/30">
                        <XCircle className="mr-2 h-4 w-4" /> Rejeitar
                      </Button>
                    </>
                  )}
                  ...
                  {sol.status === "APROVADO_AGENDA" && !isCompras && (
                    <span className="text-xs text-blue-400 flex items-center gap-1"><Clock className="h-3 w-3" /> Aguardando confirmação do Compras</span>
                  )}
                </div>
              </div>
            );
